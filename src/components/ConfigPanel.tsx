...
export const ConfigPanel: React.FC<ConfigPanelProps> = ({
  ...
  return (
    <div id="config-panel" className="flex flex-col gap-5 bg-slate-900 border border-slate-800 rounded-2xl p-4 md:p-6 text-slate-100 shadow-xl">
      {/* Header */}
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-800 pb-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="inline-flex items-center gap-1.5 text-xs font-medium px-3 py-1.5 rounded-lg bg-emerald-950/60 hover:bg-emerald-900/80 text-emerald-300 border border-emerald-700/60 transition-colors"
      ...
      {lastGeneratedForPrefix !== undefined ? (
        <div className="bg-gradient-to-r from-sky-950/50 via-slate-900 to-sky-950/30 border border-sky-500/30 rounded-xl p-3.5 flex flex-wrap items-center justify-between gap-3 text-xs">
          <div className="flex items-center gap-2.5">
            <div className="w-8 h-8 rounded-lg bg-sky-500/20 border border-sky-500/40 flex items-center justify-center text-sky-300 flex-shrink-0">
              <History className="w-4 h-4" />
            </div>
            <div>
              <div className="text-slate-200 font-medium flex items-center gap-1.5">
                Últim número generat per a <strong className="font-mono text-sky-300">{cleanPrefix}</strong>:
                <span className="font-mono bg-slate-800 px-2 py-0.5 rounded text-white font-bold border border-slate-700">
                  {formatCode(lastGeneratedForPrefix, config.prefix, config.digits)}
                </span>
              </div>
              <p className="text-slate-400 text-[11px] mt-0.5">
                Proposta automàtica: seguir a partir del <strong className="font-mono text-emerald-400 font-bold">{formatCode(nextProposedForPrefix, config.prefix, config.digits)}</strong>
              </p>
            </div>
          </div>
        ...
        <div className="bg-slate-950/60 border border-slate-800 rounded-xl p-3 flex items-center gap-2.5 text-xs text-slate-400">
          <History className="w-4 h-4 text-slate-500 flex-shrink-0" />
          <span>
            Codi <strong className="text-slate-200 font-mono">{cleanPrefix || 'sense prefix'}</strong>: no té generacions prèvies registrades. S'inicia automàticament des del número <strong className="text-white font-mono">{formatCode(1, config.prefix, config.digits)}</strong>.
          </span>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="flex flex-col gap-1.5">
          <label htmlFor="input-prefix" className="text-xs font-medium text-slate-300">
            Prefix del Codi:
          </label>
          <input
            id="input-prefix"
            ...
            className="bg-slate-950 border border-slate-800 rounded-lg px-3 py-2 text-sm text-slate-100 focus:outline-none focus:border-sky-500 font-mono"
            placeholder="IMV2627"
          />
        ...
        {/* Start Number */}
        <div className="flex flex-col gap-1.5">
          <label htmlFor="input-start-number" className="text-xs font-medium text-slate-300">
            Número d'inici:
          </label>
          <input
        ...
        {/* Digits padding */}
        <div className="flex flex-col gap-1.5">
          <label htmlFor="input-digits" className="text-xs font-medium text-slate-300">
            Dígits de farciment:
          </label>
          <input
            id="input-digits"
        ...
        {/* Quantity */}
        <div className="flex flex-col gap-1.5">
          <label htmlFor="input-count" className="text-xs font-medium text-slate-300">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 p-3 rounded-xl bg-emerald-950/30 border border-emerald-500/30 text-xs">
        <div className="flex items-center gap-2 text-emerald-300">
          <FoldHorizontal className="w-4 h-4 text-emerald-400" />
        ...
        {config.pairSpacingMm !== 0 && (
          <button
            ...
            className="px-2.5 py-1 bg-emerald-600 hover:bg-emerald-500 text-slate-950 font-semibold rounded text-xs transition-colors"
          >
            Fixar a 0 mm (tocant)
          </button>
        )}
      ...
      <div className="bg-slate-950/70 border border-slate-800 rounded-xl p-3.5 flex flex-wrap items-center justify-between gap-3 text-xs">
        <div className="flex items-center gap-2 flex-wrap">
          <span className="text-slate-400 font-medium">Rang a generar:</span>
          <span className="font-mono bg-sky-950/80 text-sky-300 border border-sky-800/80 px-2 py-0.5 rounded font-semibold">
            {startPad}
          </span>
          <span className="text-slate-500">fins a</span>
          <span className="font-mono bg-sky-950/80 text-sky-300 border border-sky-800/80 px-2 py-0.5 rounded font-semibold">
            {endPad}
          </span>
          <span className="text-slate-400 ml-1">({config.count} targetes)</span>
        </div>
        ...
        <div className="flex items-center gap-4 text-slate-300 font-mono text-[11px]">
          <span className="flex items-center gap-1">
            ...
            <strong className="text-white">{layout.pairsPerPage}</strong> parelles / full A4
          </span>
          <span className="flex items-center gap-1">
            <FileCheck className="w-3.5 h-3.5 text-amber-400" />
            <strong className="text-white">{layout.totalPages}</strong> fulls A4 en total
          </span>
        </div>
      </div>
      ...
      <div className="border-t border-slate-800 pt-3">
        ...
        {showAdvanced && (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mt-3 bg-slate-950/40 p-4 rounded-xl border border-slate-800/80 text-xs">
            <div className="flex flex-col gap-1">
              <label htmlFor="input-margin" className="text-slate-400">Marge de pàgina (mm):</label>
              <input
                id="input-margin"
                type="number"
                min="0"
            ...
            <div className="flex flex-col gap-1">
              <label htmlFor="input-spacing" className="text-slate-400">Separació entre files (mm):</label>
              <input
                id="input-spacing"
                type="number"
                min="0"
            ...
            <div className="flex flex-col gap-1">
              <label htmlFor="input-pair-spacing" className="text-slate-400">Separació davant-darrere (mm):</label>
              <input
