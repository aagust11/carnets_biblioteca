        ...
        <div className="p-6 overflow-y-auto flex flex-col gap-6">
          ...
          <div className="flex flex-col gap-2 bg-slate-950/60 p-4 rounded-xl border border-slate-800">
            <div className="flex items-center justify-between text-xs">
              ...
              <span className="font-mono text-sky-400 font-bold">{state.progress}%</span>
            </div>
          ...
          {isFinished && state.fileName && (
            <div className="bg-emerald-950/30 border border-emerald-500/30 rounded-xl p-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h4 className="text-sm font-semibold text-white">Nom del fitxer generat:</h4>
                  <p className="text-xs font-mono text-emerald-300 mt-0.5 break-all">{state.fileName}</p>
              ...
              <div className="flex items-center gap-2 w-full sm:w-auto">
                <button
                  ...
                  className="flex-1 sm:flex-none inline-flex items-center justify-center gap-2 px-4 py-2.5 bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-semibold rounded-xl shadow-lg transition-colors text-xs"
                >
                ...
                <button
                  ...
                  className="inline-flex items-center gap-1.5 px-3 py-2.5 bg-slate-800 hover:bg-slate-700 text-slate-200 rounded-xl border border-slate-700 transition-colors text-xs"
          ...
          <div className="flex flex-col gap-2">
            ...
            <div
              ref={logContainerRef}
              className="bg-slate-950 border border-slate-800 rounded-xl p-3.5 font-mono text-xs text-slate-300 h-44 overflow-y-auto space-y-1.5 scrollbar-thin select-text"
            >
              {state.logs.length === 0 ? (
                <div className="text-slate-600 italic">Esperant inici del procés...</div>
              ) : (
                state.logs.map((log) => (
                  <div
                    key={log.id}
        ...
        <div className="flex items-center justify-between px-6 py-4 border-t border-slate-800 bg-slate-950/50">
          {state.isGenerating && onCancel ? (
            <button
              type="button"
              onClick={onCancel}
              className="text-xs px-3 py-1.5 rounded-lg text-rose-400 hover:bg-rose-500/10 border border-rose-500/30 transition-colors"
            >
